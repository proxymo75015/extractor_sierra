...
TEST_CASE("Odd-start audio without primer uses odd stream") {
  fs::path tmpDir = fs::temp_directory_path();
  fs::path input = tmpDir / "odd_start_noprimer_route.rbt";
  fs::path outDir = tmpDir / "odd_start_noprimer_route_out";
  if (fs::exists(outDir)) {
  ...
  for (const auto &block : blocks) {
    ...
    robot::RobotExtractorTester::processAudioBlock(extractor, payload,
                                                   block.position);
  }
  ...
  REQUIRE_FALSE(find_alignment(evenStream, firstBlock.samples).has_value());
  auto oddIndex = find_alignment(oddStream, firstBlock.samples);
