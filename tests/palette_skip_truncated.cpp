...
TEST_CASE("Palette skip beyond file size throws") {
    fs::path tmpDir = fs::temp_directory_path();
    fs::path input = tmpDir / "trunc_palette_skip.rbt";
    fs::path outDir = tmpDir / "trunc_palette_skip_out";
    fs::create_directories(outDir);

    auto data = build_header(10); // reserve palette data but do not write it
    {
