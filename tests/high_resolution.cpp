...
static std::vector<uint8_t> build_header(uint16_t x, uint16_t y) {
...
TEST_CASE("High user-configured resolution is accepted") {
    const uint16_t x = 10000;
    const uint16_t y = 8000;

    fs::path tmpDir = fs::temp_directory_path();
    fs::path input = tmpDir / "high_res.rbt";
    auto data = build_header(x, y);
    {
        std::ofstream out(input, std::ios::binary);
        out.write(reinterpret_cast<const char*>(data.data()), static_cast<std::streamsize>(data.size()));
    }
    fs::path outDir = tmpDir / "high_res_out";
    ...
    REQUIRE(RobotExtractorTester::xRes(extractor) == x);
    REQUIRE(RobotExtractorTester::yRes(extractor) == y);
}
